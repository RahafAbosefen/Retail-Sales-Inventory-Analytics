...
const DashboardCards = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/daily_sales_summary")
      .then((res) => res.json())
      .then((data) =>
        setDailySummary({
    ...
    socket.on("daily_sales_summary", (data) =>
      setDailySummary({
        total_orders: data.total_orders,
        total_sales: data.total_sales,
      })
    );
    socket.on("update-alerts", (data) => setAlerts(data));
            ...
            >
              ...
              <div
                style={{
                  ...
                  inset: 0,
                  borderRadius: "1rem",
                  background: `linear-gradient(135deg, ${color}0D, transparent)`, // 5% opacity
                  pointerEvents: "none",
                }}
              />
